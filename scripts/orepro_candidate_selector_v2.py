...
REQUIRED_COLUMNS = {
    ...
    "race_no",
    "field_size",
    "safe_pre_race_odds_available",
    "strict_history_coverage",
    "safe_feature_coverage",
    "first_time_starter_share",
    "timestamp_sensitive_current_features_used",
}

# Fail closed if any post-race/final-market field is supplied at all.
...
# Only these columns are allowed to leave the selector. Extra harmless metadata
# may be accepted as input only if explicitly added here in a future prereg version.
OUTPUT_COLUMNS = [
    "race_id",
    "race_date_jst",
    "venue",
...
def validate_headers(fieldnames: list[str] | None) -> list[str]:
    ...
    for original, low in zip(names, lower):
        if any(tok.lower() in low for tok in FORBIDDEN_COLUMN_TOKENS):
            errs.append(f"FORBIDDEN_POSTRACE_OR_FINAL_COLUMN:{original}")

    # Fail closed on unknown columns so a new upstream field cannot silently
    # become available to later steps without a new prereg/version.
...
def main() -> int:
    ...
    out = {
        "selector_id": "OREPRO-WEEK1-CANDIDATE-SELECTOR-V2",
        "selection_rule": "data quality only; forbidden outcome/payout/final-odds columns are rejected before row evaluation",
