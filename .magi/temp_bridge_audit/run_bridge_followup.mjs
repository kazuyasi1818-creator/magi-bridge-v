...
for(const e of evidence){if(expected[e.name]&&e.client_sha256!==expected[e.name])throw new Error(`EVIDENCE_HASH_MISMATCH ${e.name} ${e.client_sha256}`)}
...
const topic=`T009R confirmatory Checker C v3のゼロベース再監査。今回の証拠は18件adversarial self-testと完全対応する固定snapshotのみを使う。candidate SHA=3cb1ff60df8463e63cf113b350099e80029a349f9e0270145c83d070792ef4d8、prereg snapshot raw SHA=00f72b616bd14dea6a9befcb45b15cdbf3bf15c3f99d8537eda178ec20456e5c、schema SHA=2c19b35d2a72b224e6c96d52395bbc42db9c5625b90baffcae9373e48af5e84d。重要な版管理事故として、Libraryに同名T009R_Confirmatory_Prereg_DRAFT_v0.5.jsonでraw SHA=7639e5feb27bee4564e9f53931d90f188444a4f22edc3c76be1751f7ead1e5a5の別内容が存在し、JSON意味差はmachine_reconciliation.checker_C_artifactがv3_candidate_final.py対v3_candidate.pyの1項目。これは再シリアライズ差ではなくsemantic content differenceなので、同名v0.5再利用禁止・今後の編集はv0.6-DRAFT以降とする。Bridgeは00f72... snapshotだけを監査対象とする。前回Actions run 32639578452は『hash mismatch』ではなくTRANSPORT_DECODE_FAIL_BEFORE_HASH_CHECKで、base64 decodeで停止しSHA照合にもBridge投入にも到達していない。この失敗記録を成功記録とともに保存する。v2監査由来の必須9 ID集合とv3 self-test ID集合の対応を件数ではなくID setとして評価する。Ra>1 secondary diagnosticがFULL/A/B各partition、implementation A/B双方で実出力されるサンプルも確認する。修正リストの答え合わせではなく、新しく追加したprereg/hash binding、expected-manifest binding、例外処理、schema v2、FULL=A+B、feasibility、relative+absolute tolerance、Ra>1診断が新しい攻撃面を作っていないかコードから再監査する。科学的閾値・Gate・SUPPORTED条件は変更禁止。preregは意図的にDRAFTで、expected_preoutcome_manifest_requirement.locked_sha256はhuman-only pre-outcome manifest作成前なのでTBD。したがって全trialのLOCK可否ではなく、Checker C v3コードがhuman anchorを埋めた後にLOCK候補へ昇格できる技術状態かを判定する。コードレベルblockerが無ければChecker C candidate auditとしてPASSしてよいが、trial/prereg全体のLOCKはhuman anchor・custody完了まで禁止と明記する。`;
