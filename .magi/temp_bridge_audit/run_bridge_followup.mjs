...
const contextObj={
  ...
  source_sha256:{
    id_mapping:expected['T009R_v3_v2_adversarial_id_mapping.json'],
    transport_failure:expected['T009R_transfer_failure_run_32639578452.json'],
    ra_diagnostic:expected['T009R_Ra_gt_1_diagnostic_sample.json'],
    prereg_collision_incident:expected['T009R_prereg_v0_5_collision_incident_20260823.json'],
    summary_reaudit_verified_but_not_embedded_due_bridge_5_file_limit:expected['T009R_checker_C_v2_to_v3_summary_reaudit.json']
  },
...
const evidence=[
  {name:'T009R_checker_C_v3_candidate_final.py',type:'text/plain',content:raw['T009R_checker_C_v3_candidate_final.py']},
...
try{
 ...
 const out={run_kind:'TEMP_BRANCH_FRESH_PRODUCTION_BRIDGE_REAUDIT_EXACT_SNAPSHOT',production_url:BASE,pre_bridge_hash_verification:'PASS',prereg_snapshot_sha256:expected['T009R_Confirmatory_Prereg_DRAFT_v0.5_snapshot_00f72b61.json'],checker_v3_sha256:expected['T009R_checker_C_v3_candidate_final.py'],context_sha256:sha(context),evidence:evidence.map(e=>({name:e.name,sha256:e.client_sha256,chars:e.content.length})),evidence_total_chars:totalChars,spent_usd:spentUsd,spent_yen:spentUsd*FX,auto_revisions:auto,final_structured:v?.structured||null,transcript,completed_at:new Date().toISOString()};fs.writeFileSync('bridge_followup_result.json',JSON.stringify(out,null,2));console.log(JSON.stringify(out,null,2));
}catch(err){const out={run_kind:'TEMP_BRANCH_FRESH_PRODUCTION_BRIDGE_REAUDIT_EXACT_SNAPSHOT',pre_bridge_hash_verification:'PASS',prereg_snapshot_sha256:expected['T009R_Confirmatory_Prereg_DRAFT_v0.5_snapshot_00f72b61.json'],checker_v3_sha256:expected['T009R_checker_C_v3_candidate_final.py'],error:String(err?.stack||err),spent_usd:spentUsd,spent_yen:spentUsd*FX,transcript,completed_at:new Date().toISOString()};fs.writeFileSync('bridge_followup_result.json',JSON.stringify(out,null,2));console.error(JSON.stringify(out,null,2));process.exit(1)}
