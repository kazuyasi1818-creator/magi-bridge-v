...
const files=[
  {path:'T009R_expected_manifest_selfcheck_v1_1.py',name:'T009R_expected_manifest_selfcheck_v1_1.txt',type:'text/plain'},
  {path:'test_T009R_expected_manifest_selfcheck_v1_1_adversarial.py',name:'test_T009R_expected_manifest_selfcheck_v1_1_adversarial.txt',type:'text/plain'},
  {path:'T009R_expected_manifest_selfcheck_v1_1_selftest_report.json',name:'T009R_expected_manifest_selfcheck_v1_1_selftest_report.json',type:'application/json'},
  {path:'T009R_checker_C_v3_1_to_v3_1_1.patch',name:'T009R_checker_C_v3_1_to_v3_1_1_diff.txt',type:'text/plain'},
  {path:'T009R_selfcheck_v1_1_hardening_report.json',name:'T009R_selfcheck_v1_1_hardening_report.json',type:'application/json'}
];
const sha=s=>crypto.createHash('sha256').update(s,'utf8').digest('hex');
const evidence=files.map(f=>{const content=fs.readFileSync(`${dir}/${f.path}`,'utf8');return{name:f.name,type:f.type,size:Buffer.byteLength(content),content,client_sha256:sha(content),source_path:f.path}});
...
try{
 ...
 const out={run_kind:'T009R_SELFCHECK_V1_1_CHECKER_V3_1_1_REAUDIT',production_url:BASE,evidence:evidence.map(e=>({name:e.name,source_path:e.source_path,sha256:e.client_sha256,chars:e.content.length})),spent_usd:spentUsd,spent_yen:spentUsd*FX,auto_revisions:auto,final_structured:v?.structured||null,transcript,completed_at:new Date().toISOString()};
