#!/usr/bin/env python3
"""
T009R deterministic reconciliation checker C v2 CANDIDATE.
...
def validate_bundle(b, expected, label):
    ...
    if inf.get("bootstrap_seed") != seed_for(b["prereg_id"], impl) if impl in EXPECTED_IMPL_IDS else True:
        errors.append({"scope": f"{label}.inference", "error": "wrong_bootstrap_seed", "expected": seed_for(b["prereg_id"], impl) if impl in EXPECTED_IMPL_IDS else None, "actual": inf.get("bootstrap_seed")})
...
def main():
    ...
    result = {
        "checker_id": "T009R_CHECKER_C_V2_CANDIDATE",
        "tolerance_abs": TOL,
